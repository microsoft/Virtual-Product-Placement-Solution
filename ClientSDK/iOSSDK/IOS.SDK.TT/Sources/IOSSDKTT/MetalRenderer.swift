...
//  IOS.SDK.TT
//
//  Re-exports `IOSSDKCore` so consumers only need `import IOSSDKTT`.
//
//  Mirrors `IOS.SDK.Native/MetalRenderer.swift` so TT picks up the same
//  ad-overlay rendering path (overlay pipeline, per-slot uniforms, mipmapped
//  ad textures with anisotropic sampling). The only structural difference is
//  on the *video* side: TT decodes via TTSDK and can deliver either BGRA or
//  NV12 pixel buffers, so the video pass picks pipeline + textures per-frame.
//
...
public class MetalRenderer {
    ...
    public var onAdTexturesReady: (() -> Void)?
    ...
    public var frameDumper: FrameDumper?
    ...
    @MainActor
    public func setAdTimeline(_ timeline: AdTextureTimeline, forAdIndex adIndex: String) {
    ...
    @MainActor
    public func loadAdTextures(for parser: AdManifestParser) {
    ...
    public func makePlaneTexture(from pixelBuffer: CVPixelBuffer,
                          plane: Int,
    ...
    public func renderToDrawable(pixelBuffer: CVPixelBuffer, in view: MTKView) {
    ...
    /// (the default) and this is non-zero, the renderer sizes its output
    /// buffer + MSAA color attachment so that a single buffer pixel maps 1:1
    /// to a screen pixel inside the aspect-fit displayed video region. This
    /// matches what MTKView gives the Native SDK and avoids the extra
    /// upscale blur introduced by `AVSampleBufferDisplayLayer` when the
    /// buffer is smaller than its displayed region.
    ///
    /// Set `useDeviceResolutionForRasterization: false` to force the legacy
    /// behavior — rasterize at the input `CVPixelBuffer`'s resolution
    /// regardless of the display size. The same fallback kicks in
    /// automatically when `displayPixelSize == .zero` (e.g. the view hasn't
    /// been laid out yet).
    public func renderToPixelBuffer(
        _ input: CVPixelBuffer,
        overlays: [OverlayElement] = [],
        videoSize: CGSize = .zero,
        currentVideoPTS: TimeInterval = 0,
        displayPixelSize: CGSize = .zero,
        useDeviceResolutionForRasterization: Bool = true
    ) -> CVPixelBuffer? {
