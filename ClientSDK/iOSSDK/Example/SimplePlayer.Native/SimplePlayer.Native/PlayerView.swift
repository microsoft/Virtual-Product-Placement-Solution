...
struct PlayerView: View {
    ...
    // Built lazily in `.task` so the renderer's shader / pipeline-state
    // compilation never runs on the navigation-push critical path.
    // Constructing it inline as a `@State` default would force
    // `MTLCreateSystemDefaultDevice`, `makeDefaultLibrary` and
    // `makeRenderPipelineState` to complete on the main thread before
    // SwiftUI even started the push animation — visible as a ~hundreds-
    // of-ms hang on first navigation. With `nil` here the view appears
    // instantly and the renderer comes online a frame or two later.
    @State private var renderer: (any Renderer)? = nil
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack {
                ...
                ZStack {
                    Color.black
                    if viewModel.hasVideo, let renderer {
                        let metalVideoView = MetalVideoView(renderer: renderer, viewModel: viewModel)
                        if viewModel.session.videoSize.width > 0 {
                            metalVideoView.aspectRatio(viewModel.session.videoSize, contentMode: .fit)
                        } else {
                            metalVideoView
                        }
                    }
                    ...
                    VStack {
                        HStack {
                            ...
                            VStack(alignment: .trailing, spacing: 2) {
                                ...
                                Text("Display Frame (MUL): \(viewModel.nominalFPS > 0 ? Int(viewModel.framePTS * Double(viewModel.nominalFPS)) : 0)")
                                ...
                                Text("Count: \(viewModel.frameIndex)")
                        ...
                        VStack(spacing: 8) {
                            ...
                            HStack(spacing: 20) {
                                ...
                                Button {
                                    ...
                                    if viewModel.hasVideo && !viewModel.session.isAdsReady {
                                        // Ad textures still downloading — show a
        ...
        .task {
            // Build the Metal renderer off the navigation-push critical
            // path. Pipeline-state compilation can take a noticeable beat
            // on the main thread; doing it here means the push animation
            // has already started by the time we pay that cost.
            if renderer == nil {
