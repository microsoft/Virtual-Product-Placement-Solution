...
void Tracker::track(const cv::Mat im_prev, const cv::Mat im_gray, const std::vector<cv::Point2f> &points_prev,
                    std::vector<cv::Point2f> &points_tracked, std::vector<unsigned char> &status)
{
    ...
    if (points_prev.size() > 0)
    {
        ...
        //Traverse std::vector backward so we can remove points on the fly
