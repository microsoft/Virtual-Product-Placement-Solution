...
void removeRepeated(const std::vector<cv::KeyPoint> &skeypoints, const std::vector<cv::KeyPoint> &mkeypoints, std::vector<cv::DMatch> &good_matches, float pos_threshold = 2.0)
{
    ...
    for (size_t i = 0; i < good_matches.size();)
    {
        ...
        {
            if (fabs(mkeypoints[existed_matches[j].trainIdx].pt.x - mkeypoints[good_matches[i].trainIdx].pt.x) < pos_threshold || fabs(skeypoints[existed_matches[j].queryIdx].pt.x - skeypoints[good_matches[i].queryIdx].pt.x) < pos_threshold)
            {
                bExisted = true;
                break;
            }
        ...
        while (true)
        {
            ...
            for (int i = 0; i < map_size; i++)
            {
                ...
                for (int j = 0; j < map_size; j++)
                    sum += brother_matrix[mapId[i]][mapId[j]];
                if (sum > maxv)
                {
                    maxv = sum;
                    maxid = mapId[i];
            ...
            for (int i = 0; i < map_size; i++)
            {
                if (brother_matrix[maxid][mapId[i]] > 0)
                    mapId[id++] = mapId[i];
            }
            map_size = maxv;
        }
